...
class TestPoint2D(unittest.TestCase):
    ...
    def test_iadd_Point2D_w_Point2D(self):
    ...
    def test_sub_Point2D_from_Point2D_actual_Vector2D_instance(self):
        expected_vector = Vector2D(-3, 4)
        p0 = Point2D(0, 2)
        p1 = Point2D(3, -2)
        actual_vector = p0 - p1
...
class TestPointVectorInteraction(unittest.TestCase):
    ...
    def test_iadd_Vector_w_Point(self):
        """test Vector += Point"""
        p = Point(4, 6, 7, 2)
        v = Vector(-6, -5, 1, 3)
        with self.assertRaises(TypeError) as e:
            v += p
        expected = "Can only mutate a Vector via addition with a Vector"
        self.assertEqual(expected, str(e.exception))

    def test_iadd_Point_w_Vector(self):
        """test Vector += Point"""
        expected = Point(-2, 1, 0, 3)
    ...
    def test_isub_sub_Point_from_Vector(self):
    ...
    def test_isub_sub_Point_from_Point(self):
...
class TestPoint2DVector2DInteraction(unittest.TestCase):
    ...
    def test_iadd_Point2D_w_Vector2D_instance_mutated(self):
        _ = Point2D(-4, 5)
        actual = Vector2D(2, 1)
        expected_id = id(actual)
        actual += Vector2D(-6, 4)
    ...
    def test_sub_Vector2D_from_Point2D(self):
        """Point - Vector = Point"""
    ...
    def test_isub_sub_Point2D_from_Vector2D(self):
        # Vector -= Point does not make sense
        expected = "Can only mutate a Vector or a Point by subtracting a Vector"
        v0 = Vector2D(-1, -4)
        p0 = Point2D(1, 4)
